...
output.textContent = 'Let the game begins';
...
function playRound() {
  ...
  if (result == 0) {
    output.textContent = `Its a tie! You both picked ${playerChoice}`;
  } else if (result == 1) {
    output.textContent = `You won! ${playerChoice} won over ${compChoice.Value}`;
    playerScore++;
  } else if (result == 2) {
    output.textContent = `Computer beats you with ${compChoice.Value}!`;
    compScore++;
  }
}
...
function playGame() {
  ...
  computer.textContent = `Computer Score: ${compScore}`;
  if (playerScore == 5) {
    output.textContent = 'WOW YOU WON THE GAME! CONGRATS!';
    playerScore = 0;
    compScore = 0;
    player.textContent = `Player Score: ${playerScore}`;
    computer.textContent = `Computer Score: ${compScore}`;
  } else if (compScore == 5) {
    output.textContent = 'AW YOU LOST!';
    playerScore = 0;
    compScore = 0;
    player.textContent = `Player Score: ${playerScore}`;
    computer.textContent = `Computer Score: ${compScore}`;
